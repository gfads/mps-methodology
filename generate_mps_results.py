...
for i in range(0, 4):
    ...
    parameters = list(product(ACCURACY_METRICS, METRICS, MODEL_NAMES, W[i], WS[i]))
...
parameters = list(product(ACCURACY_METRICS, METRICS, ML_MODELS, TARGET_VARIABLES, WINDOW_SIZES))
save_accuracy_metrics(parameters)
save_figures(parameters)
concatenate_results(TARGET_VARIABLES, METRICS, ML_MODELS, WINDOW_SIZES, ACCURACY_METRICS)
...
parameters = list(product(ACCURACY_METRICS, METRICS, ML_MODELS, TARGET_VARIABLES))

save_accuracy_metrics_one_model(parameters)
save_figures_dynamic_one_model(parameters)
concatenate_results_with_one_result_dynamic(TARGET_VARIABLES, METRICS, ML_MODELS, ACCURACY_METRICS)
everyone_folder_with_one_result_dynamic(TARGET_VARIABLES, METRICS, ACCURACY_METRICS)
everyone_folder_with_one_result_dynamic_all(METRICS, ACCURACY_METRICS)